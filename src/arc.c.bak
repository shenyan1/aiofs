 ...
 */
void
print_state (struct __arc *cache)
{
...
static void
__arc_hash_init (struct __arc *cache)
{
    uint64_t i;
...
__arc_hash_lookup (struct __arc *cache, uint64_t id, uint64_t offset,
		   lmutex_t ** mem_lock)
{
    struct __arc_list *iter;
    uint64_t key;
...
static void
__arc_hash_fini (struct __arc *cache)
{
...
void
__arc_object_init (struct __arc_object *obj, unsigned long size)
{
...
static struct __arc_object *
__arc_state_lru (struct __arc_state *state)
{
...
static struct __arc_object *
__arc_move_state (struct __arc *cache, struct __arc_state *state1,
...
//	  printf("obj's read state=%d",obj->read_state);
	  ...
	  if (state2 == &cache->mrug || state2 == &cache->mfug)
		...
		if (state2 == &cache->mrug)
		    ...
		    printf ("evict:moving into the mfug ");
...
static struct __arc_object *
__arc_move (struct __arc *cache, struct __arc_object *obj,
...
static void
__arc_replace (struct __arc *cache, int ismfug)
{
...
static void
__arc_adjust (struct __arc *cache)
{
    printsize (cache);
    /* case A:
     */
...
#if 0
static void
__arc_balance (struct __arc *cache, unsigned long size)
{
...
/* Create a new cache. */
struct __arc *
__arc_create (struct __arc_ops *ops, uint64_t c)
{
    int i;
...
void
__arc_destroy (struct __arc *cache)
{
    //   struct __arc_list *iter;
    int i;
...
struct __arc_object *
__arc_lookup (struct __arc *cache, uint64_t id, uint64_t offset)
{
#define IN_MFUG 1
    struct __arc_object *obj;
    lmutex_t *hash_lock;
