...
struct __arc_hash {
	...
	pthread_mutex_t hash_mutexes[ARC_MUTEXES+1];
	uint64_t hm_mask;
};
...
struct __arc_ops {
    ...
    int (*cmp) (struct __arc_object *obj, uint64_t id,uint64_t offset);
    ...
    struct __arc_object *(*create) (uint64_t id,uint64_t offset);
     ...
     */
    int (*fetch_from_disk)(uint64_t id,uint64_t offset,struct __arc_object *obj);
...
typedef struct __arc {
    ...
    uint64_t c, p;
	pthread_mutex_t arc_lock;
	pthread_mutex_t arc_balock;
