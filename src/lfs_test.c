...
test_write (int id, char *buffer, uint64_t size, uint64_t offset)
{
    ...
    if (file_write (id, buffer, size, offset))
      {
	  printf ("write failed id=%d,size=%" PRIu64 ",offset=%" PRIu64 "",
		  id, size, offset);
	  exit (1);
      }
    return 1;
...
test_read (int id, char *buffer, uint64_t size, uint64_t offset)
{
    if (file_read (id, buffer, size, offset))
      {
	  printf ("read failed id=%d,size=%" PRIu64 ",offset=%" PRIu64 "", id,
		  size, offset);
	  return -1;
      }
    return 0;
