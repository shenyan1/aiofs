...
int LoadSpaceEntry(){
   ...
   uint64_t off,size;
	...
	pread(lfs_n.fd,buffer,2*sizeof(uint64_t),lfs_n.off); 
	off = buffer[0];
	size = buffer[1];
...
int lfs_init()
{
	...
	for(i=0;i<10<<10;i++)
		lfs_n.f_table[i].meta_table = malloc(FILE_ENTRYS*sizeof(uint64_t));
	lfs_n.lfs_cache = cache_create("arc_cache", 1<<20, sizeof(char*),NULL, NULL,arc_size);
	lfs_n.lfs_obj_cache = cache_create("obj_cache",sizeof(struct object), sizeof(char*),NULL, NULL,arc_size);
