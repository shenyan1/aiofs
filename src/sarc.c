...
struct __sarc;
struct __sarc_object *__sarc_hash_lookup (uint64_t offset, uint64_t id,
					  lmutex_t ** mem_lock);
struct object *sarc_getobj (struct __sarc_object *e);
inline void __sarc_hash_insert (struct __sarc *cache, uint64_t key,
...
void
sarc_move_state (struct __sarc_object *obj)
{
...
void
readandmru (uint64_t id, uint64_t offset)
{
    int id = range.fileid;
    struct __sarc_object *obj;
    lmutex_t *hash_lock;
...
static void
print_obj (struct __sarc_object *obj, char *str)
{
    struct object *objc;
...
static void
__sarc_hash_init (struct __sarc *cache)
{
    uint64_t i;
...
inline void
__sarc_hash_insert (struct __sarc *cache, uint64_t key,
		    struct __sarc_object *obj, lmutex_t * hash_lock)
{
    ...
    {
	struct __sarc_object *obj =
	    __arc_list_entry (iter, struct __sarc_object, hash);
...
__sarc_hash_lookup_nolock (uint64_t offset, uint64_t id)
{
    struct __arc_list *iter;
    uint64_t key;
    key = lfs_arc_hash (id, offset);
    unsigned long hash =
	lfs_n.sarc_cache->ops->hash (key) & lfs_n.sarc_cache->hash.ht_mask;
...
inline lmutex_t *
get_lfs_hashlock (uint64_t offset, uint64_t id)
{
...
inline int
need_lock_neighbor (uint64_t offset, uint64_t id)
{
    return get_lfs_hashlock (offset,
			     id) != get_lfs_hashlock (offset - LFS_BLKSIZE,
						      id);
}

struct __sarc_object *
__sarc_hash_lookup (uint64_t offset, uint64_t id, lmutex_t ** mem_lock)
{
    struct __arc_list *iter;
...
static void
__arc_hash_fini (struct __sarc *cache)
{
...
void
__sarc_object_init (struct __sarc_object *obj, uint64_t offset, int id,
		    unsigned long size)
{
	...
	obj_prev = __sarc_hash_lookup_nolock (id, offset - LFS_BLKSIZE);
      ...
      {
	  obj_prev =
	      __sarc_hash_lookup (id, offset - LFS_BLKSIZE, &hash_lock);
      }
    if (obj_prev)
      {
...
gethash_lock (struct __sarc *cache, struct __sarc_object *_obj)
{
...
static struct __sarc_object *
__arc_state_lru (struct __sarc_state *state)
{
...
#if 0
static struct __sarc_object *
__arc_move_state (struct __sarc *cache, struct __arc_state *state1,
...
#endif
static struct __sarc_object *
__sarc_move (struct __sarc *cache, struct __sarc_object *obj,
	     struct __sarc_state *state, int flag)
{
    if (obj->state)
      {
	  if (flag == OBJ_UNLOCK)
...
#if 0
static void
__arc_adjust (struct __sarc *cache)
{
    printsize (cache);
    /* case A:
     */
...
/* Create a new cache. */
struct __sarc *
__sarc_create (struct __sarc_ops *ops, uint64_t c)
{
    int i;
...
static inline void
list_destroy (struct __sarc_state *state, struct __sarc *cache)
{
...
void
__sarc_destroy (struct __sarc *cache)
{
    //   struct __arc_list *iter;
    int i;
...
static inline void
arc_stat_hit_update (struct __sarc *cache)
{
...
static inline void
arc_stat_update (struct __sarc *cache)
{
...
inline void
sarc_read_done (struct __sarc_object *obj)
{
    mutex_enter (&obj->obj_lock, __func__);
...
int
isbottom (struct __sarc_state *state, struct __sarc_object *obj)
{
...
struct __sarc_object *
__sarc_lookup (struct __sarc *cache, uint64_t id, uint64_t offset)
{
#define IN_MFUG 1
		...
		if (isbottom (obj->state, obj))
		  {
		      ...
		      t = 2.0 * cache->seqMiss * cache->tail_stat /
			  cache->seq.size;
		      if (t > LargeRatio)
			{
			    cache->adapt = 1;
 ...
 */
	  if (obj == NULL)
	    ...
	    {
		printf ("in arc_lookup obj=%p,obj->read_state=%d", obj,
			obj->read_state);
		assert (0);
	    }
      }
